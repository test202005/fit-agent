...
> Review 方法：对照 [prd-iter1-intent.md](prd-iter1-intent.md) v3 与 [architecture-iter1.md](architecture-iter1.md) v3 逐项核对；单测与 stub 回归由评审人本机复跑验证；评测报告按 prompt_hash 时间线交叉核对
> 结论先行：**工程质量合格，但 iter-1 退出门禁暂不能判过**——P0 一项（版本证据链断裂）+ P1 两项修复并重跑一次干净的 locked 验收后，才能关账
...
- [ ] locked 使用规则（仅最终验收运行、补位盲写）已写入文档
