...
- 单元测试 114 passed；四套 stub 全绿（意图 58 / 抽取 28 / 查询 22 / 工具 28）
...
| 方向 | 为什么 | 大致范围 |
|---|---|---|
| **补 token 成本维度** | 架构对比缺的最后一块，最便宜 | `response.usage` 透出，对比脚本累加 |
| **多模型对比** | 直接对上「模型对比分析」这条岗位要求，几乎不需要新方法 | 同一套评测集跑两个模型，出对比报告 |
